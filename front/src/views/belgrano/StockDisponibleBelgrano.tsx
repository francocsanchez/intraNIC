...
export default function StockDisponibleBelgrano() {
  ...
  const items = data?.data ?? EMPTY_STOCK_BELGRANO;
    ...
    return (
      <div className="font-preset w-full bg-muted px-2 py-3">
        <section className="rounded-lg border border-destructive/30 bg-card p-3 text-card-foreground shadow-sm">
          <h1 className="text-lg font-semibold tracking-tight text-foreground">Error al cargar el stock de Belgrano</h1>
    ...
    <div className="font-preset w-full space-y-3 bg-muted px-2 py-3">
      <section className="overflow-hidden rounded-lg border border-border bg-card text-card-foreground shadow-sm">
        <div className="px-3 py-3">
          ...
          <h1 className="mt-1 text-xl font-semibold tracking-tight text-foreground">Stock disponible</h1>
        </div>
        ...
        {marcasDisponibles.map((filtro) => {
          ...
          return (
            <button
              ...
              className={`h-9 rounded-md border text-xs font-medium transition-colors ${activo ? "border-border bg-primary text-primary-foreground" : "border-border bg-card text-muted-foreground hover:bg-secondary hover:text-foreground"}`}
            >
              {filtro}
            </button>
          );
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="bg-muted text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">
              <tr>
                {['Interno', 'Dominio', 'Marca', 'Version', 'Color', 'Ano', 'Km', 'Recepcion', 'Precio venta'].map((heading) => (
            ...
            <tbody className="divide-y divide-border">
              {itemsFiltrados.map((item) => (
                <tr key={`${item.interno}-${item.marca}-${item.fechaRecepcion}`} className="hover:bg-muted">
                  ...
                  <td className="px-3 py-1.5 text-muted-foreground">{item.dominio || "-"}</td>
