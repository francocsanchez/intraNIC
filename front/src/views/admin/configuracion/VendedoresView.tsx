    ...
    return (
      <div className="w-full px-4 py-6">
        <div className="rounded-2xl border border-red-200 bg-white p-6 shadow-sm">
          <h2 className="text-lg font-semibold tracking-tight text-gray-900">Error al cargar vendedores</h2>
    ...
    <div className="w-full px-4 py-6 space-y-6">
      ...
      <section className="grid grid-cols-1 gap-4 md:grid-cols-3">
        ...
        <article className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
          ...
          <p className="mt-2 text-3xl font-semibold tracking-tight text-gray-900">{items.filter((item) => item.estado === 1).length}</p>
        </article>

        <article className="rounded-2xl border border-gray-200 bg-white px-6 py-4 shadow-sm">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Tipos</p>
          ...
          >
            <div className="px-3 first:pl-0">
              ...
              <p className="text-lg font-semibold text-gray-900">{tiposStats.nuevo}</p>
            </div>

            <div className="px-3">
              <p className="text-xs text-gray-500 uppercase">Usado</p>
              <p className="text-lg font-semibold text-gray-900">{tiposStats.usado}</p>
            </div>

            <div className="px-3">
              <p className="text-xs text-gray-500 uppercase">Plan</p>
              <p className="text-lg font-semibold text-gray-900">{tiposStats.plan}</p>
            </div>

            <div className="px-3 last:pr-0">
              <p className="text-xs text-gray-500 uppercase">Postventa</p>
              <p className="text-lg font-semibold text-gray-900">{tiposStats.posventa}</p>
            </div>
          </div>
        </article>
      </section>
      ...
      <section className="overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-sm">
        <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">Configuración general de tipos y sucursal</p>
          </div>
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full text-sm">
            <thead className="border-b border-gray-200 bg-gray-50">
              <tr>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Código</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Vendedor</th>
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Nuevo</th>
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Usado</th>
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Plan</th>
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Posventa</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Email Tecnom</th>
                <th className="px-4 py-3 text-center text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Estado</th>
                <th className="px-4 py-3 text-left text-xs font-semibold uppercase tracking-[0.18em] text-gray-500">Sucursal</th>
              </tr>
            </thead>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.codigo} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-4 py-3 text-gray-700">{item.emailTecnom || "-"}</td>

                  <td className="px-4 py-3 text-center">
                    <span
                      className={[
                        "inline-flex rounded-md border px-2.5 py-1 text-xs font-medium",
                        item.estado === 1 ? "border-green-200 bg-green-50 text-green-700" : "border-red-200 bg-red-50 text-red-700",
                      ].join(" ")}
                    >
