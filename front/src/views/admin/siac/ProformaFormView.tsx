...
export default function ProformaFormView() {
  ...
  if (versionesQuery.error instanceof Error) {
    return (
      <div className="font-preset w-full bg-muted px-2 py-3"><section className="rounded-lg border border-destructive/30 bg-card p-3 shadow-sm"><h1 className="text-lg font-semibold text-foreground">No se pudo cargar el formulario de proformas</h1><p className="mt-1 text-sm text-destructive">{versionesQuery.error.message}</p>
        </section>
      </div>
    );
  }
  ...
  return (
    <div className="font-preset w-full space-y-3 bg-muted px-2 py-3"><section className="rounded-lg border border-border bg-card text-card-foreground shadow-sm"><div className="flex flex-col gap-3 px-3 py-3 md:flex-row md:items-start md:justify-between">
          <div>
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Proformas</p><h1 className="mt-1 text-xl font-semibold tracking-tight text-foreground">Nueva proforma</h1><p className="mt-1 text-sm text-muted-foreground">
              Cargá los datos comerciales, agregá una o varias unidades y revisá los cálculos antes de guardar.
            </p>
          </div>
          <Link to={paths.convencional.proformas} className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-foreground">
            <ArrowLeft size={16} />
            Volver al listado
          </Link>
        </div>
      ...
      <section className="rounded-lg border border-border bg-card p-3 text-card-foreground shadow-sm"><div className="grid grid-cols-1 gap-3 xl:grid-cols-12">
          <label className="flex flex-col gap-1 text-sm font-medium text-foreground xl:col-span-4">
            Señores
            <input
              type="text"
              value={senores}
      ...
      <section className="rounded-lg border border-border bg-card p-3 text-card-foreground shadow-sm">
        <div className="flex items-center justify-between gap-3">
          <div>
            <h2 className="text-base font-semibold text-foreground">Unidades</h2><p className="mt-1 text-sm text-muted-foreground">Cada unidad generará automáticamente tres renglones en el PDF.</p>
          </div>
          <button
            type="button"
        ...
        <div className="mt-3 divide-y divide-border">
          {unidades.map((unidad, index) => (
            <article key={`unidad-${index}`} className="py-3 first:pt-0">
              <div className="mb-3 flex items-center justify-between"><h3 className="text-sm font-semibold text-foreground">Unidad {index + 1}</h3>
                <button
                  type="button"
                  onClick={() => setUnidades((current) => current.filter((_, itemIndex) => itemIndex !== index))}
                  disabled={unidades.length === 1}
                  className="inline-flex items-center gap-2 rounded-md bg-destructive px-2 py-1 text-xs font-semibold text-destructive-foreground hover:bg-destructive/90 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  <Trash2 size={14} />
                  Quitar
                </button>
        ...
        <div className="mt-3 flex justify-end">
          <button
            ...
            className="inline-flex items-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-semibold text-primary-foreground hover:bg-primary/90 disabled:cursor-not-allowed disabled:opacity-60"
          >
            <Save size={16} />
            Guardar proforma
          </button>
      ...
      <section className="overflow-hidden rounded-lg border border-border bg-card text-card-foreground shadow-sm"><div className="border-b border-border px-3 py-3"><h2 className="text-base font-semibold tracking-tight text-foreground">Vista previa de cálculos</h2><p className="mt-1 text-sm text-muted-foreground">Patentamiento se muestra exento de IVA. Los ítems en 0 no se incluyen como renglón adicional.</p>
        </div>
