...
export default function StockReservasConvencional() {
    ...
    return reservasPorSucursal
      .map(([sucursal, reservas]) => {
        const filtradas =
          modeloActivo === "TODOS"
            ? reservas
            : reservas.filter((item) => item.modelo === modeloActivo);
  ...
  const totalFiltrado = useMemo(() => {
    return sucursalesFiltradas.reduce(
      (acc, [, reservas]) => acc + reservas.length,
      0,
    );
  }, [sucursalesFiltradas]);
          ...
          <div
            ...
            style={{
              gridTemplateColumns: `repeat(${Math.max(
                Object.keys(data?.resumen.sucursales ?? {}).length,
                1,
              )}, minmax(0,1fr))`,
            }}
          >
            {Object.entries(data?.resumen.sucursales ?? {}).map(
              ([sucursal, total]) => (
                ...
                {reservas.map((item) => (
                  <tr
                    ...
                    className={[
                      ...
                      diasReserva(item.fechaReserva) > 2
