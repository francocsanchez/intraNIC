...
export default function StockDisponibleConvencional() {
  ...
  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["stockDisponible", "convencional"],
    queryFn: getStockDisponibleConvencional,
    refetchOnWindowFocus: true,
  ...
  const ubicacionSeleccionada = ubicacionesDisponibles.includes(ubicacionActiva) ? ubicacionActiva : "TODAS";

  const itemsVisibles = useMemo(() => {
    if (ubicacionSeleccionada === "TODAS") return itemsFiltrados;
    return itemsFiltrados.filter((item) => normalizarUbicacion(item.ubicacion) === ubicacionSeleccionada);
  }, [itemsFiltrados, ubicacionSeleccionada]);
    ...
    <div className="font-preset w-full space-y-3 bg-muted px-2 py-3">
      <section className="overflow-hidden rounded-lg border border-border bg-card text-card-foreground shadow-sm">
        <div className="px-3 py-3">
          ...
          <h1 className="text-xl font-semibold tracking-tight text-foreground">Stock disponible</h1>
        </div>
              ...
              {ubicacionesDisponibles.map((ubicacion) => (
                <button
                  ...
                  className={[
                    ...
                    ubicacionSeleccionada === ubicacion ? "bg-card text-foreground" : "text-muted-foreground hover:bg-secondary hover:text-foreground",
                  ].join(" ")}
                >
                  {ubicacion}
                </button>
          ...
          <table className="min-w-full text-sm">
            <thead className="border-b border-border bg-muted">
              <tr>
                ...
                <th className="px-3 py-2 text-left text-xs font-semibold uppercase tracking-[0.18em] text-muted-foreground">Observaciones</th>
              </tr>
            </thead>
