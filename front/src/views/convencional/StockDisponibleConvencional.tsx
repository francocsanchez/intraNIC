...
const FILTROS_PRIORITARIOS: ModeloFiltro[] = [
  "TODOS",
  "HILUX",
  "SW4",
  "HIACE",
...
export default function StockDisponibleConvencional() {
  ...
  const filtrosDisponibles = useMemo(() => {
    ...
    return FILTROS_PRIORITARIOS.filter(
      (filtro) => filtro === "TODOS" || existentes.has(filtro),
    );
  }, [items]);
        ...
        <article className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex h-full flex-col items-center justify-center">
            <p className="text-6xl font-semibold tracking-tight text-gray-900">
              {resumen?.total ?? resumenDinamico.total}
            </p>
        ...
        <div className="flex flex-col gap-3 border-b border-gray-200 px-6 py-4 md:flex-row md:items-center md:justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">
              {modeloActivo === "TODOS"
                ? "Listado completo de unidades disponibles"
                : `Listado filtrado por modelo: ${modeloActivo}`}
            </p>
          </div>
              ...
              {itemsFiltrados.map((item) => (
                <tr
                  ...
                  className={[
                    ...
                    item.vendedorReserva === "3MES QUE VIENE"
                ...
                >
                  ...
                  <td className="px-4 py-2 text-gray-700 text-center">
                    <span
                      className={`inline-block px-2 py-1 text-xs font-medium rounded-md border border-slate-200 ${textToColor(item.color)}`}
                    >
                      {item.color}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-2 text-gray-700">
                    {formatDate(item.fechaRecepcion)}
                  </td>
                  <td className="px-4 py-2 text-gray-700">
                    {item.vendedorReserva}
                  </td>
                </tr>
              ))}
