...
import { getStockReventaConvencional } from "@/api/convencional/stockAPI";
...
export default function StockDisponibleReventas() {
  const [modeloActivo, setModeloActivo] = useState<ModeloFiltro>("TODOS");

  const { data, isLoading, isError, error } = useQuery({
    queryKey: ["stockReventa", "convencional"],
    queryFn: getStockReventaConvencional,
            ...
            <tbody>
              {itemsFiltrados.map((item) => (
                ...
                >
                  ...
                  {item.ubicacion ? <td className="px-4 py-2 text-gray-700">{item.ubicacion}</td> : <td className="px-4 py-2 text-gray-700">EN PRODUCCION</td>}
