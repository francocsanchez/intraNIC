import { runSsiVentasHotAlertCron } from "../services/ssiVentasHotAlertCron.service";
...
const executeIfNeeded = async () => {
  ...
  if (isRunning) {
    console.warn("[ssi-ventas-hot-alert-cron] la ejecucion anterior sigue en curso, se omite este ciclo");
    return;
  }
  ...
  try {
    await runSsiVentasHotAlertCron();
  } finally {
    isRunning = false;
  }
};

export const startSsiVentasHotAlertJob = () => {
  console.log(
    `[ssi-ventas-hot-alert-cron] programado todos los dias a las ${String(JOB_HOUR).padStart(2, "0")}:${String(JOB_MINUTE).padStart(2, "0")} (${JOB_TIMEZONE})`,
  );
